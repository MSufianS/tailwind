...
export default plugin(({ addComponents, theme }) => {
  const config = theme(`ale.${key}`) satisfies ButtonConfig
  ...
  addComponents({
    '.ale-button': {
      ...
      [`@apply font-${config.font.family} font-${config.font.weight} leading-5 no-underline`]:
        {},
      //State styles
      [`@apply ale-focus transition-${config.transition.property} duration-${config.transition.duration} disabled:opacity-60 disabled:cursor-not-allowed hover:shadow-none`]:
        {},
      // #endregion
      ...
      '&.ale-button-pastel': {
        //Pastel:default
        '&.ale-button-default, &.ale-button-default-contrast': {
          //Text color
        ...
        //Pastel:muted
        '&.ale-button-muted, &.ale-button-muted-contrast': {
          //Text color

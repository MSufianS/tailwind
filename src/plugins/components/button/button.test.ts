...
describe('Button', () => {
  test('Should render slot', () => {
    ...
    expect(document.body.querySelector('.ale-button')?.outerHTML)?.toContain(
      'Hello world',
    )
  })
  ...
  test('Should not render children on loading', () => {
    ...
    expect(
      document.body.querySelector('.ale-button')?.outerHTML,
  ...
  test('Should have no axe violations', async () => {
    ...
    expect(
      await axe(document.body.querySelector('.ale-button')!.outerHTML),
    )?.toHaveNoViolations()
  })
})

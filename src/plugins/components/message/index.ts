...
export default plugin(({ addComponents, theme }) => {
  const config = theme(`ale.${key}`) satisfies MessageConfig
  ...
  addComponents({
    ...
    '.ale-message': {
      ...
      //Message:close
      '.ale-message-close': {
        [`@apply ale-focus flex cursor-pointer items-center justify-center shrink-0 h-${config.close.size.inner.height} w-${config.close.size.inner.width} ${config.close.rounded}`]:
          {},
        //Color

...
const config = {
  theme: {
    ...
    extend: {
      ...
      animation: {
        'ale-tooltip-x': `ale-tooltip-x 300ms ease-out forwards`,
        'ale-tooltip-y': `ale-tooltip-y 300ms ease-out forwards`,
      },
    },
  },
}
...
export default plugin(({ addComponents, theme }) => {
  const config = theme(`ale.${key}`) satisfies TooltipConfig

  const tooltip = `data-ale-tooltip`
  const position = `data-ale-tooltip-position`
